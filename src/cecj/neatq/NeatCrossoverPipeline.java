...
public class NeatCrossoverPipeline extends BreedingPipeline {
	...
	NeatNeuralNetworkIndividual[] parents;
	
	public NeatCrossoverPipeline()
	{
		parents = new NeatNeuralNetworkIndividual[2];
	}
		...
		for (int i = start; i < n + start; i++) {
			if (sources[0] == sources[1]) {
				...
				if (!(sources[0] instanceof BreedingPipeline)) {
					parents[0] = (NeatNeuralNetworkIndividual) (parents[0].clone());
					parents[1] = (NeatNeuralNetworkIndividual) (parents[1].clone());
				}
			} else {
